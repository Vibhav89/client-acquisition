...
export function scoreOpportunity(
  ...
  const evidenceScore = profile.evidence.length === 0
    ? 0
    : (profile.evidence.filter((item) => wanted.has(normalize(item.skill)))
        .reduce((sum, item) => sum + Math.max(0, Math.min(1, item.strength)), 0) /
        profile.evidence.length) * 20;

  const budgetScore = opportunity.budget?.currency.toUpperCase() === "USD"
    ? Math.min(20, ((opportunity.budget.max ?? opportunity.budget.min ?? 0) >= (profile.minimumHourlyUsd ?? 0) ? 20 : 5))
    : 0;
